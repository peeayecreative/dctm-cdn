jQuery(function($){
    ...
    $.fn.flipper = function(action, options) {
      ...
      var settings = $.extend({
        // defaults.
        HideAllzero: $flipper.data('hideallzero'),
        HideDaysZero: $flipper.data('hidedayszero'),
        ...
        SingularDays: $flipper.data('days'),
        SingularHours: $flipper.data('hours'),
        SingularMinutes: $flipper.data('minutes'),
        SingularSeconds: $flipper.data('seconds'),
        preload: true
      }, options );
      ...
      if(action === 'init'){
        ...
        templateParts.forEach(function(part, index){
          ...
          if(typeof labelsArray[index] !== 'undefined'){
            if(part === 'dd' || part === 'ddd'){
              $part.append('<p class="et_pb_with_border flipper-label flipper-label-days label">' + labelsArray[0] + '</p>');
            }
            if(part === 'HH'){
              $part.append('<p class="et_pb_with_border flipper-label flipper-label-hours label">' + labelsArray[1] + '</p>');    
            }
            if(part === 'ii'){
              $part.append('<p class="et_pb_with_border flipper-label flipper-label-minutes label">' + labelsArray[2] + '</p>'); 
            }
            if(part === 'ss'){
              $part.append('<p class="et_pb_with_border flipper-label flipper-label-seconds label">' + labelsArray[3] + '</p>'); 
            }
        ...
        if(settings.preload){
          setFlipperDate($flipper, settings.datetime, false, settings.HideDaysZero, settings.HideAllzero);
        }
  
        setInterval(function(){
          setFlipperDate($flipper, settings.datetime, true, settings.HideDaysZero, settings.HideAllzero);
        }, 1000);
      ...
      function setFlipperDate($flipper, dateString, animate, HideDaysZero, HideAllzero){
     ...
     //     upsizeToParent($flipper);
          setFlipperDate($flipper, settings.datetime, false, HideDaysZero, HideAllzero);
        }
        ...
        if(days_str < 2){
          $flipper.find('.flipper-label-days').html(settings.SingularDays);   
        }

        if(hours_str < 2){
          $flipper.find('.flipper-label-hours').html(settings.SingularHours);   
        }

        if(minutes_str < 2){
          $flipper.find('.flipper-label-minutes').html(settings.SingularMinutes);   
        }

        if(seconds_str < 2){
          $flipper.find('.flipper-label-seconds').html(settings.SingularSeconds);   
        }

         var hideZero =  HideDaysZero=="on" && days_str[0] == 0 ? " timer_display_none" : " " ;
         var hideHourZero = HideAllzero =="on" && hours_str[0] == 0 ? " timer_display_none" : " " ;
         var hideMintero = HideAllzero =="on" && minutes_str[0] == 0 ? " timer_display_none" : " " ;
         var hideSecZero = HideAllzero =="on" && seconds_str[0] == 0 ? " timer_display_none" : " " ;
        ...
        if(animate){
          ...
          $flipper.find('.flipper-HH').find('.flipper-digit:eq(0)').addClass(hideHourZero);
          ...
          $flipper.find('.flipper-ii').find('.flipper-digit:eq(0)').addClass(hideMintero); 
          ...
          $flipper.find('.flipper-ss').find('.flipper-digit:eq(0)').addClass(hideSecZero);
          ...
          $flipper.find('.flipper-ddd').find('.flipper-digit:eq(0)').addClass(hideZero);
        }
        else {
          ...
          $flipper.find('.flipper-ddd').find('.flipper-digit:eq(0)').addClass(hideZero);
          console.log(days_str[0],"days_str[0]",days_str[1],"days_str[1]");
  ...
  jQuery(function($){
    ...
    $('.ditp_countdown_timer').each(function (i, item) {

      var classfilter = jQuery(item).attr('class').split(/\s+/);

      $.each(classfilter, function (index, item) {

        if (item.match(/ditp_countdown_timer_/g)) {
          mainClass = "." + item;
      ...
      var FlipCounter = $(window)[0][`object${mainClass.substr(1, mainClass.length)}`];

      var CustomFlipTimer = FlipCounter.flip_counter_class !== 'undefined'? FlipCounter.flip_counter_class :" ";
     // console.log(FlipCounter.flip_counter_class,"check");
    //  if(typeof(FlipCounter.flip_counter_class) !== 'undefined'){

      $('#Flipper_'+CustomFlipTimer).flipper('init');
      //test
  //  }
    });
